...
import { getEnglishTutorial, getTutorial, getTutorialSummaries } from "@/lib/tutorial-data";
...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
  ...
  const english=await getEnglishTutorial(tutorial.slug);
  ...
  return {title,description,keywords:[tutorial.category,...tutorial.tools,"tutorial AI","hướng dẫn AI"],alternates:{canonical:path,languages:english?{"vi-VN":path,en:`/en${path}`,"x-default":path}:{"vi-VN":path,"x-default":path}},authors:[{name:tutorial.author_name||siteConfig.author}],openGraph:{type:"article",url:path,title,description,publishedTime:tutorial.published_at,modifiedTime:tutorial.updated_at||tutorial.published_at,authors:[tutorial.author_name||siteConfig.author],section:tutorial.category,tags:tutorial.tools,images:image?[{url:image,alt:tutorial.title}]:undefined},twitter:{card:"summary_large_image",title,description,images:image?[image]:undefined}};
}
