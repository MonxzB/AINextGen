...
export default async function AdminAnalyticsPage({ searchParams }: { searchParams: Promise<{ days?: string }> }) {
  ...
  return <>
    ...
    <div className="mt-5 flex gap-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/[.07] p-4 text-sm leading-6"><ShieldCheck className="mt-0.5 shrink-0 text-emerald-400" size={20}/><p><b>Đang lọc lượt truy cập rác:</b> chỉ nhận request cùng website, giới hạn 15 request/phút, không đếm lại cùng trang trong một phiên 30 phút và có hạn mức bổ sung tại database.</p></div>
