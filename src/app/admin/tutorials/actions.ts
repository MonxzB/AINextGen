...
export async function saveTutorial(_state:TutorialActionState,formData:FormData):Promise<TutorialActionState>{
  ...
  const englishPublished=formData.get("is_english_published")==="on";
  if(englishPublished&&((input.title_en?.length||0)<5||(input.excerpt_en?.length||0)<20||(input.content_en?.length||0)<10))return {error:"Bản tiếng Anh chưa đủ nội dung để xuất bản.",fieldErrors:{title_en:["Cần tiêu đề, mô tả và nội dung tiếng Anh đầy đủ."]}};
  ...
  if(result.error)return {error:result.error.code==="23505"?"Slug đã tồn tại. Hãy chọn slug khác.":result.error.message.includes("title_en")||result.error.message.includes("is_english_published")?"Database chưa có trường song ngữ. Hãy chạy migration 020_bilingual_articles.sql.":result.error.message.includes("content_blocks")?"Database chưa có block editor. Hãy chạy migration 005_article_block_editor.sql.":result.error.message.includes("author_name")||result.error.message.includes("source_references")||result.error.message.includes("reviewed_at")?"Database chưa có trường tác giả và kiểm chứng. Hãy chạy migration 007_content_experience.sql.":result.error.message};
  if(result.data?.status!==status)return {error:"Database chưa lưu đúng trạng thái bài viết. Hãy tải lại trang và thử lại."};
  revalidateTag("tutorials");revalidatePath("/");revalidatePath("/tutorials");revalidatePath("/en");revalidatePath("/en/tutorials");revalidatePath("/admin");revalidatePath("/admin/tutorials");if(input.id){revalidatePath(`/tutorials/${values.slug}`);revalidatePath(`/en/tutorials/${values.slug}`);}
