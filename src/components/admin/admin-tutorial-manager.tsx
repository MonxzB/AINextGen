...
export function AdminTutorialManager({ rows, configured, initialFilters, saved, translationResult }: { rows:AdminTutorialRow[]; configured:boolean; initialFilters:Filters; saved:boolean; translationResult?:string }) {
  ...
  return <>
    ...
    {translationResult==="translated"&&<div className="mt-3 rounded-xl border border-cyan-500/25 bg-cyan-500/10 p-4 text-sm font-semibold text-cyan-500">Google đã tự động tạo và lưu bản tiếng Anh.</div>}
    ...
    <div className="mt-4 grid gap-3 md:hidden">
      {pagination.rows.map((row) => <article key={row.id} className="card p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><Link href={`/admin/tutorials/${row.id}?category=${encodeURIComponent(row.category)}`} className="line-clamp-2 font-black hover:text-brand-700">{row.title}</Link><p className="mt-1 truncate text-xs text-black/40">/{row.slug}</p></div>{row.is_featured && <Sparkles className="shrink-0 text-brand-700" size={18}/>}</div><div className="mt-4 flex flex-wrap items-center gap-2"><StatusBadge status={row.status}/><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${row.is_english_published?"bg-cyan-500/15 text-cyan-500":"bg-white/5 text-black/40"}`}>{row.is_english_published?"EN đã đăng":"EN chờ dịch"}</span><span className="rounded-full bg-white/5 px-2.5 py-1 text-xs text-black/50">{row.category}</span><span className="text-xs text-black/40">{row.duration_minutes} phút</span></div><div className="mt-4 flex items-center justify-between border-t border-white/10 pt-3"><span className="text-xs text-black/40">Sửa {date.format(new Date(row.updated_at))}</span><div className="flex items-center gap-1"><Link href={`/admin/tutorials/${row.id}?category=${encodeURIComponent(row.category)}`} className="rounded-lg px-2 py-1.5 text-sm font-bold text-brand-700 hover:bg-white/5">Sửa</Link>{row.status === "published" && <Link href={`/tutorials/${row.slug}`} target="_blank" className="rounded-lg p-2 text-black/50 hover:bg-white/5" aria-label={`Xem ${row.title}`}><Eye size={16}/></Link>}<DeleteTutorialButton id={row.id} title={row.title}/></div></div></article>)}
    </div>
