...
import { CheckCircle2, Eye, FilePenLine, Languages, LoaderCircle, Plus, Search, Sparkles, Zap } from "lucide-react";
import { translatePendingTutorials } from "@/app/admin/tutorials/actions";
...
import type { AdminTutorialRow, TranslationActionState } from "@/types/admin";
...
const initialTranslationState:TranslationActionState={};

export function AdminTutorialManager({ rows, configured, initialFilters, saved, translationResult }: { rows:AdminTutorialRow[]; configured:boolean; initialFilters:Filters; saved:boolean; translationResult?:string }) {
  const [translationState,translationAction,translating]=useActionState(translatePendingTutorials,initialTranslationState);
  ...
  const pagination = getPageSlice(filtered, page);
  ...
  function clearFilters() {const next={q:"",status:"all",category:"all"};setPage(1);setFilters(next);syncUrl(next,"push");}
  ...
  return <>
    ...
    {translationResult==="translated"&&<div className="mt-3 rounded-xl border border-cyan-500/25 bg-cyan-500/10 p-4 text-sm font-semibold text-cyan-500">Google đã tạo bản tiếng Anh. Bản này đang chờ bạn kiểm duyệt trước khi công khai.</div>}
    {translationResult==="failed"&&<div className="mt-3 rounded-xl border border-amber-500/25 bg-amber-500/10 p-4 text-sm text-amber-500"><b>Bản tiếng Việt đã lưu.</b> Bản EN chưa dịch được; kiểm tra Cloud Translation API, billing và API key.</div>}
    {translationResult==="not-configured"&&<div className="mt-3 rounded-xl border border-amber-500/25 bg-amber-500/10 p-4 text-sm text-amber-500">Server chưa nhận GOOGLE_TRANSLATE_API_KEY nên chưa chạy dịch tự động.</div>}
    ...
    {configured&&<form action={translationAction} className="card mt-5 flex flex-wrap items-center justify-between gap-3 p-4"><div><p className="font-black">Dịch tự động bài cũ</p><p className="mt-1 text-xs text-black/45">Mỗi lượt tạo tối đa 3 bản dịch chờ kiểm duyệt để tránh timeout và vượt quota.</p>{translationState.success&&<p className="mt-2 text-xs font-bold text-emerald-500">{translationState.success} {translationState.remaining?"Còn "+translationState.remaining+" bài.":""}</p>}{translationState.error&&<p className="mt-2 text-xs font-bold text-red-600">{translationState.error}</p>}</div><button type="submit" disabled={translating} className="btn-secondary gap-2 disabled:opacity-50">{translating?<LoaderCircle size={17} className="animate-spin"/>:<Languages size={17}/>} {translating?"Đang dịch...":"Dịch 3 bài tiếp theo"}</button></form>}
