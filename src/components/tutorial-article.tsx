...
export function TutorialArticle({tutorial,related=[],preview=false}:{tutorial:Tutorial;related?:TutorialSummary[];preview?:boolean}){
  ...
  const articleSchema={"@context":"https://schema.org","@type":"Article",headline:tutorial.title,description:tutorial.excerpt,datePublished:tutorial.published_at,dateModified:updated,inLanguage:"vi-VN",mainEntityOfPage:{"@type":"WebPage","@id":url},author:{"@type":"Person",name:author},publisher:{"@type":"Organization",name:siteConfig.name,alternateName:siteConfig.alternateName,url:siteConfig.url,logo:{"@type":"ImageObject",url:absoluteUrl("/icon")}},articleSection:tutorial.category,keywords:[tutorial.category,...tutorial.tools].join(", "),image:[image]};
  ...
  return <div className="container-page py-10">
    ...
    <header className="mx-auto mt-10 max-w-4xl text-center"><span className="rounded-full border border-brand-500/30 bg-brand-500/10 px-3 py-1.5 text-sm font-bold text-brand-700">{tutorial.category}</span><h1 className="mt-6 text-4xl font-black leading-tight sm:text-5xl">{tutorial.title}</h1><div className="mt-6 flex flex-wrap justify-center gap-4 text-sm text-black/45"><span>{levels[tutorial.difficulty]}</span><span className="flex items-center gap-1"><Clock size={16}/>{tutorial.duration_minutes} phút đọc</span><span className="flex items-center gap-1"><UserRound size={16}/>{author}</span><time dateTime={reviewed}>Biên tập {dateFormat.format(new Date(reviewed))}</time></div></header>
    <div className="mx-auto mt-12 grid max-w-6xl gap-8 lg:grid-cols-[minmax(0,1fr)_280px]">
      <div className="min-w-0 space-y-6"><article id="tutorial-content" className="card p-6 sm:p-8 lg:p-10"><ArticleContent blocks={tutorial.content_blocks} legacyContent={tutorial.content} intro={tutorial.excerpt}/></article>
        <section className="card p-6 sm:p-8" aria-labelledby="editorial-trust"><div className="flex items-start gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-brand-500/15 text-brand-700"><BadgeCheck size={24}/></span><div><h2 id="editorial-trust" className="text-lg font-black">Biên tập bởi {author}</h2><p className="mt-2 text-sm leading-6 text-black/55">{tutorial.author_bio||"Đội ngũ AINextGen biên soạn nội dung theo hướng thực hành, nêu rõ giới hạn và cập nhật khi công cụ AI thay đổi."}</p><p className="mt-2 text-xs font-semibold text-black/40">Cập nhật biên tập: {dateFormat.format(new Date(reviewed))}</p></div></div></section>
        <section className="rounded-2xl border border-amber-400/25 bg-amber-400/10 p-6 sm:p-8" aria-labelledby="editorial-note"><h2 id="editorial-note" className="text-lg font-black">Phạm vi hướng dẫn</h2><p className="mt-2 text-sm leading-6 text-black/55">Prompt và quy trình trong bài là điểm khởi đầu để bạn tự thử nghiệm, không phải cam kết về lượt xem hay thu nhập. Tính năng, giá và chính sách của các nền tảng có thể thay đổi; hãy đối chiếu nguồn bên dưới và kiểm tra đầu ra AI trước khi sử dụng. AI có thể hỗ trợ khâu trình bày, còn bản công khai do biên tập viên AINextGen chịu trách nhiệm.</p></section>
