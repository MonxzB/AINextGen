...
export function TutorialArticle({tutorial,related=[],preview=false}:{tutorial:Tutorial;related?:TutorialSummary[];preview?:boolean}){
  ...
  const image=tutorial.cover_url||tutorial.content_blocks?.find((block)=>block.type==="image")?.url||absoluteUrl("/opengraph-image");
  ...
  return <div className="container-page py-10">
    ...
    <header className="mx-auto mt-10 max-w-4xl text-center"><span className="rounded-full border border-brand-500/30 bg-brand-500/10 px-3 py-1.5 text-sm font-bold text-brand-700">{tutorial.category}</span><h1 className="mt-6 text-4xl font-black leading-tight sm:text-5xl">{tutorial.title}</h1><p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-black/55">{tutorial.excerpt}</p><div className="mt-6 flex flex-wrap justify-center gap-4 text-sm text-black/45"><span>{levels[tutorial.difficulty]}</span><span className="flex items-center gap-1"><Clock size={16}/>{tutorial.duration_minutes} phút đọc</span><span className="flex items-center gap-1"><UserRound size={16}/>{author}</span><time dateTime={reviewed}>Kiểm chứng {dateFormat.format(new Date(reviewed))}</time></div></header>
    <div className="mx-auto mt-12 grid max-w-6xl gap-8 lg:grid-cols-[minmax(0,1fr)_280px]">
      <div className="min-w-0 space-y-6"><article className="card p-6 sm:p-8 lg:p-10"><ArticleContent blocks={tutorial.content_blocks} legacyContent={tutorial.content}/></article>
